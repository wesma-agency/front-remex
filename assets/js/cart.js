...
function updatedUIPrice(itemCart, priceHTML) {
  ...
  UI_ELEMENT.TOTAL.PRICE.forEach((item) => {
    item.textContent = TOTAL_CART.totalPrice;
  });
  UI_ELEMENT.TOTAL.COUNT.forEach((item) => {
    item.textContent = TOTAL_CART.totalCountItems;
  });
  UI_ELEMENT.TOTAL.PRICE_DISCOUNT.forEach((item) => {
    item.textContent = TOTAL_CART.totalPriceDiscount;
  });
}
...
function getTabItemsContent() {
  const tabItems = document.querySelectorAll(".cart__main .nav-btns__btn.tabs-btns__btn");
  const tabsContentArr = [];

  tabItems.forEach((item) => {
    tabsContentArr.push(item);
  });

  return tabsContentArr;
}

function renderSelect() {
  const tabsContent = getTabItemsContent();
  const selectField = document.querySelector(".cart__main .simple-select .simple-select__field");
  const selectItems = document.querySelectorAll(".cart__main .simple-select .simple-select__list .simple-select__item");
  const selectItemActive = document.querySelector(".cart__main .simple-select .simple-select__list .simple-select__item--active");

  selectItems.forEach((item, index) => {
    item.innerHTML = tabsContent[index].innerHTML;

    item.addEventListener("click", (e) => {
      selectField.innerHTML = e.target.innerHTML;
    });
  });

  selectField.innerHTML = selectItemActive.innerHTML;
}
